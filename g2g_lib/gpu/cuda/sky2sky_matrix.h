...
void get_sky2sky_matrix_v1(interpolation_parameters &parameters){
    ...
    for(size_t ch_g_idx=0; ch_g_idx<parameters.no_grid_index; ch_g_idx++){
        ...
        for(size_t ch=ch_g_idx*parameters.grid_channel_width; ch<(ch_g_idx+1)*parameters.grid_channel_width; ch++){
            ...
            printf("Local coo size = %d, Effective compression is %f\n", local_coo_size, ((float)local_coo_size)/((float)parameters.nrows));
            local_coo_size = 0;
        }//end row
...
void get_sky2sky_matrix_v3(interpolation_parameters &parameters){
    ...
    printf("Total coo size is %zu, Effective compression is %f\n", parameters.tot_size_coo, ((float)parameters.tot_size_coo)/((float)parameters.nrows*parameters.Nchan));
}

void get_sky2sky_matrix_v35(interpolation_parameters &parameters){
