			...
			PRECISION2* kernels, int2* kernel_supports, PRECISION3* vis_uvw_coords, PRECISION2* visibilities, Config *config, PRECISION2* uv_grid)
{
	...
	PRECISION2* d_kernels = NULL;
	int2* d_kernel_supports = NULL;
	PRECISION3* d_vis_uvw_coords = NULL;
	PRECISION2* d_visibilities = NULL;
	PRECISION2* d_uv_grid = NULL;


	CUDA_CHECK_RETURN(cudaMalloc(&d_kernels, sizeof(PRECISION2) * TOTAL_KERNEL_SAMPLES));
	CUDA_CHECK_RETURN(cudaMemcpy(d_kernels, kernels, sizeof(PRECISION2) * TOTAL_KERNEL_SAMPLES, cudaMemcpyHostToDevice));
	cudaDeviceSynchronize();

	CUDA_CHECK_RETURN(cudaMalloc(&d_kernel_supports, sizeof(int2) * NUM_KERNELS));
	CUDA_CHECK_RETURN(cudaMemcpy(d_kernel_supports, kernel_supports, sizeof(int2) * NUM_KERNELS, cudaMemcpyHostToDevice));
	cudaDeviceSynchronize();
	...
	CUDA_CHECK_RETURN(cudaMemset(d_uv_grid, 0, grid_square * sizeof(PRECISION2)));
	...
	gridding<<<kernel_blocks, kernel_threads>>>(d_uv_grid, d_kernels, d_kernel_supports, 
		d_vis_uvw_coords, d_visibilities, NUM_VISIBILITIES, config->oversampling,
		GRID_SIZE, config->uv_scale, config->w_scale);
	cudaDeviceSynchronize();



	CUDA_CHECK_RETURN(cudaMemcpy(uv_grid, d_uv_grid, sizeof(PRECISION2) * grid_square, cudaMemcpyDeviceToHost));
	cudaDeviceSynchronize();
	...
	CUDA_CHECK_RETURN(cudaFree(d_kernel_supports));
	CUDA_CHECK_RETURN(cudaFree(d_kernels));
}
#else
void gridding_actor(int GRID_SIZE, int NUM_VISIBILITIES, int NUM_KERNELS, int TOTAL_KERNEL_SAMPLES,
        PRECISION2* kernels, int2* kernel_supports, PRECISION3* vis_uvw_coords, PRECISION2* visibilities, Config *config, PRECISION2* uv_grid)
{
    ...
    gridding_CPU(uv_grid, (PRECISION2*) kernels, kernel_supports,
        (PRECISION3*) vis_uvw_coords, (PRECISION2*) visibilities, NUM_VISIBILITIES, config->oversampling,
        GRID_SIZE, config->uv_scale, config->w_scale);
