...
#endif
pthread_barrier_t iter_barrier_d;
int preesmStopThreads_d;

#ifdef PREESM_MD5_UPDATE
		struct rk_sema preesmPrintSema;
#endif
unsigned int launch_d(unsigned int core_id, pthread_t *thread, void* (*start_routine)(void*), void* arg) {
...
PRECISION *image_estimate,PRECISION3 *source_list,int *num_source,
  ...
  args.image_estimate = image_estimate;
  args.source_list = source_list;
  args.num_source = num_source;

  args.major_iter[0]=iter_major;
  args.delta_image = delta_image;
  args.image_out = image_out;

#ifndef _WIN32
  signal(SIGSEGV, handler_d);
  signal(SIGPIPE, handler_d);
#endif
  ...
  preesmStopThreads_d = 0;
  pthread_barrier_init(&iter_barrier_d, NULL, _PREESM_NBTHREADS_);
#ifdef PREESM_MD5_UPDATE
  ...
  for (int i = 0; i < _PREESM_NBTHREADS_; i++) {
    if (i != _PREESM_MAIN_THREAD_) {
      if (launch_d(i, &coreThreads[i], coreThreadComputations[i],&args)) {
        printf("Error: could not launch_d thread %d\n", i);
        return ;
