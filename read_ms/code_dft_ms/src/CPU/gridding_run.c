    ...
    for (unsigned int vis_index = 0; vis_index < num_vis; vis_index++)
    {
        ...
        for(int grid_v = min_grid_point.y; grid_v <= max_grid_point.y; ++grid_v)
        {
            ...
            for(int grid_u = min_grid_point.x; grid_u <= max_grid_point.x; ++grid_u)
            {
                ...
                int kernel_size = 108800;

                if (kernel_index < 0 || kernel_index >= kernel_size) {
                    printf("kernel_index=%d (max index ?), oversampling=%d\n", kernel_index, oversampling);
                    printf("plane_index = %d, w_kernel_offset = %d\n", plane_index, w_kernel_offset);

                    fprintf(stderr, "Invalid kernel_index: %d (kernel_size=%d)\n", kernel_index, kernel_size);
                    exit(1);
                }
